      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/60 p-7 border border-slate-100 space-y-5">
          ...
          <div className="space-y-2">
            {OAUTH_PROVIDERS.map(p => (
              <button
                ...
                className={`w-full flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl border text-sm font-medium text-slate-700 transition-all disabled:opacity-60 ${p.className}`}
              >
                {oauthLoading === p.id ? (
          ...
          <p className="text-center text-sm text-slate-500">
            No account?{' '}
            <Link href="/signup" className="text-indigo-500 font-semibold hover:text-indigo-600">
              Create one free
            </Link>
          </p>
        </div>
