...
export default function AssetsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setProfile(p)
    const { data } = await supabase.from('assets').select('*').eq('household_id', p.household_id).order('created_at', { ascending: false })
    setAssets(data || [])
  }, [supabase])
  ...
  const openEdit = (a: Asset) => { setEditing(a); setForm({ name: a.name, category: a.category, value: String(a.value), currency: a.currency, institution: a.institution || '', notes: a.notes || '' }); setOpen(true) }

  const save = async () => {
    if (!profile) return
    setLoading(true)
    const payload = { name: form.name, category: form.category, value: parseFloat(form.value) || 0, currency: form.currency, institution: form.institution, notes: form.notes, user_id: profile.id, household_id: profile.household_id }
    if (editing) {
  ...
  return (
    <div>
      <Topbar title="Assets" subtitle={`${assets.length} assets · ${formatCurrency(total)}`} userName={profile?.full_name}
            ...
            {assets.map((a) => (
              <Card key={a.id} className="hover:shadow-md transition-shadow">
                <CardContent className="pt-5">
                  ...
                  <p className="text-2xl font-bold text-indigo-600">{formatCurrency(a.value, a.currency)}</p>
                  <p className="text-xs text-slate-400 mt-1">Updated {formatDate(a.updated_at)}</p>
